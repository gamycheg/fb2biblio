...
function processBookData($file, $pdo) {
    ...
    $book_id = addBook($title, $annotation, $lang, $date, $cover,$file);
...
function processFB2File($filePath, $pdo) {
    ...
    try {
        ...
        $book_id = processBookData($file, $pdo);

        if ($book_id) {
            if (substr($book_id, 0, 4) == "OLD_") {
                logMessage(date("d-m-Y H:i:s")." - Книга уже существует в базе с ID: ".substr($book_id, 4));
            } else {
                logMessage(date("d-m-Y H:i:s")." - Книга успешно добавлена с ID: $book_id");
            }
        } else {
