...
function generateOPDSAuthor($pdo, $author) {
    ...
    $query = "SELECT id,lang,title,annotation,`date`,`path`,author_name,author_id FROM fullbookauthor WHERE author_id = :author ORDER BY title ASC";
    $stmt = $pdo->prepare($query);
...
function generateOPDSGenres($pdo, $genres) {
    ...
    //Иконка
    $xml->addChild('icon', 'http://nd.alabuzya.ru/opds/img/book.png');

    // Запрос к базе данных для получения книг по автору
    $query = "SELECT * FROM fullbookgenres WHERE genre_name = :genre AND LEFT(title, 1) = :letter ORDER BY title ASC";
    $stmt = $pdo->prepare($query);
    ...
    while ($book = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $thm->addAttribute('href', 'data:image/jpeg;base64,' . $book['cover']);
        $thm->addAttribute('type', 'image/jpg');
        $img = $entry->addChild('link');
        $img->addAttribute('rel', 'http://opds-spec.org/image');
        $img->addAttribute('href', 'data:image/jpeg;base64,' . $book['cover']);
...
function generateOPDS($pdo, $page) {
    ...
    if($letter == null){
        ...
        if ($letter != null) {
            ...
            switch ($section) {
                case 'genres':
                    $query = "SELECT DISTINCT LEFT(title, 1) AS letter FROM fullbookgenres WHERE genre_name = :letter ORDER BY letter ASC";
                    $stmt = $pdo->prepare($query);
